...
function evenAndOddWhileLoop(max) {
  ...
  let current = 0;
  while (current < max) {
    current % 2 == 0 ? array.push(`${current}+"is even"`) : array.push(`${current}+"is odd"`);
    current++;
  }
  return array;
}
...
function evenAndOddForLoop(max) {
  let array = [];
  for (let i = 0; i < max; i++) {
    i % 2 === 0 ? array.push(`${i}+is even`) : array.push(`${i}+is Odd`);
  }
  return array;
}
